...
int acpi_table_parse_mcfg(struct sdt_header_t *hdr) {
  ...
  int nentry = (hdr->length - sizeof(*hdr)) / sizeof(struct acpi_mcfg_record);
  struct acpi_mcfg_record *record =
      (struct acpi_mcfg_record *)((uintptr_t)hdr + sizeof(*hdr) +
                                  sizeof(uint64_t));

  int ret;
  for (int i = 0; i < nentry; i++) {
    kprintf("acpi: mcfg[%d]: base 0x%x, seg %d, bus %d-%d\n", i,
