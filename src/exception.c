...
static int page_fault_handler(uint16_t irqn, struct context *ctx, void *_) {
...
static int double_fault_hanlder(uint16_t irqn, struct context *ctx, void *_) {
...
static int invalid_opcode_hanlder(uint16_t irqn, struct context *ctx, void *_) {
...
int set_exception_handlers(void) {
  ...
  for (int i = 0; i < 32; i++) {
    set_irq_handler(i, generic_exception_handler, NULL);
  }

  set_irq_handler(EXC_NUM_PAGE_FAULT, page_fault_handler, NULL);
  set_irq_handler(EXC_NUM_DOUBLE_FAULT, double_fault_hanlder, NULL);
  set_irq_handler(EXC_NUM_INVALID_OPCODE, invalid_opcode_hanlder, NULL);
