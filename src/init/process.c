...
/*         :"m"(start_task_array[next]->stack_pointer)); */
    ...
    __asm__ volatile(
            ...
            "jmp _switch_to;\n\t"
            "1:;"
            : "=m"(start_task_array[prev]->rsp),
            "=m"(start_task_array[prev]->rip)
            : "m"(start_task_array[next]->rsp),
            "m"(start_task_array[next]->rip));
}
